...
<style>
.select2-container--default .select2-results>.select2-results__options {display:none;}
</style>
    <!-- Modal -->
                            ...
                            <div class="col-md-12">
                                <div class="param-ref param-ref-withdraw  money-ref r-mt-11">
                                    ...
                                    <select  id="type" class="select2 withdraw-type" data-minimum-results-for-search="Infinity">
                                        @foreach($paymentMethods as $method)
                                            <option disabled="disabled" value="{{ $method->id }}" {{ old('type') == $method->id ? 'selected' : '' }} >{{ $method->titl }}USDT (TRC-20)</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="label-top mt-withdraw">
                                        <label class="gilroy-medium text-gray-100 mb-2 f-14 leading-17 mt-20 r-mt-amount r-mt-6" >Адрес вашего кошелька</label>
                                        <input type="text" class="form-control input-form-control input-form-control-withdraw apply-bg" name="email" id="email" onkeyup="this.value = this.value.replace(/\s/g, '')" placeholder="Убедитесь в правильности ввода кошелька. Иначе деньги будут потеряны безвозратно." data-value-missing="{{ __('This field is required') }}">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="d-flex withdraw-list-child">
            <div class="d-flex flex-wraps flex-grows-1">
                <div class="transaction-medium d-flex width-50 responsive-top">
                    ...
                    <p class="text-dark gilroy-medium f-16 mb-0 leading-20">{{ $row->paymentMethod?->titl }}</p>
                    </div>
                </div>
...
$("p").each(function() {
    var text = $(this).text();
    text = text.replace("Paypal", "USDT (TRC-20)");
    $(this).text(text);
});
</script>
<div class="mt-3">
...
@push('js')
    ...
    <script>
        ...
        var addModalHeadingText = "{{ __('Add Withdrawal Setting') }}";
