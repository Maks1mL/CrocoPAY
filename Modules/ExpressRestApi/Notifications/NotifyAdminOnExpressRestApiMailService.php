...
namespace Modules\ExpressRestApi\Notifications;

use Exception;
use App\Services\Mail\TechVillageMail;

class NotifyAdminOnExpressRestApiMailService extends TechVillageMail
{
    /**
    ...
    public function send($payment, $optional = [])
    {
        ...
        try {
            ...
            $data = [
                ...
                "{user}" => getColumnValue($payment->user) ?? __('Unregistered User'),
            ...
            $body = str_replace(['Fee (deducted from {fee_bearer}):', '{fee}'], '', $response['template']->body);
            $message = str_replace(array_keys($data), $data, $body);
