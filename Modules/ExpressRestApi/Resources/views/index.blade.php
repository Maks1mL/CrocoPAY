        ...
        @if($isMerchantAvailable && $merchant->status == 'Approved')
            ...
            <div class="transaction-order-quantity">
                <div class="d-flex justify-content-between">
                    <h3>{{ getColumnValue($merchant?->user) }}'s {{ getColumnValue($merchant, 'business_name') }}</h3>

                    <span>
                        @php
                            $amount = isset($paymentInfo['amount']) ? $paymentInfo['amount'] : 0;
                        @endphp
                        {{ moneyFormat(optional($currency)->code, formatNumber($amount, optional($currency)->id)) }}
                    </span>
                </div>
                <p>{{ __('Merchant ID') }}: #{{ $merchant->merchant_uuid ?? "" }}</p>
            </div>
            <div class="transaction-total d-flex justify-content-between">
                <h3>{{ __('Total') }} ({{ optional($currency)->code }})</h3>
                <span>{{ formatNumber($amount, optional($currency)->id) }}</span>
            </div>

            <form action="{{ route('merchant.payment_form') }}" method="get" id="paymentMethodForm">
                ...
                <input name="currency" value="{{ optional($currency)->code}}" type="hidden">
                <input name="currency_id" value="{{ optional($currency)->id}}" type="hidden">
